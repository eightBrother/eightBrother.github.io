...
function dyniframesize(down) {
	...
	var iframeDiv = $(parent.window.document).find(".l-tab-content-item");
	if (pTar && !window.opera) {
		...
		if (pTar.contentDocument && pTar.contentDocument.body.offsetHeight) {
			...
			for(var i=0;i<iframeDiv.length;i++){
				if(iframeDiv.eq(i).is(":visible")){
					iframeDiv.eq(i).find("iframe").height(pTar.contentDocument.body.offsetHeight + 60);
				}
			} 
			...
			for(var i=0;i<iframeDiv.length;i++){
				if(iframeDiv.eq(i).is(":visible")){
					iframeDiv.eq(i).find("iframe").height(pTar.Document.body.scrollHeight+40);
				}
			} 
		}
	}
    ...
    l.controls.Dialog.ligerExtend(l.core.Win, {
        ...
        _render: function ()
        {
            ...
            else if (p.url)
            {
                ...
                {
                    g.jiframe = $("<iframe frameborder='0' width=100% scrolling='yes' ></iframe>");
...
(function ($)
{
    ...
    $.ligerDefaults.TabString = {
        closeMessage: "关闭当前页",
        closeOtherMessage: "关闭其他页",
        closeAllMessage: "关闭所有页",
        reloadMessage: "刷新当前页"
    };
    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        overrideTabItem: function (targettabid, options)
        {
            ...
            {
                contentitem.html("<iframe frameborder='0' scrolling='yes' ></iframe>");
            }
            else if (content)
            {
        ...
        addTabItem: function (options)
        {
            ...
            var contentitem = $("<div class='l-tab-content-item'><div class='l-tab-loading' style='display:block;'></div><iframe frameborder='0' scrolling='yes'  ></iframe></div>");
            ...
            if (url)
            {
                iframe.attr("name", tabid)
                 .attr("id", tabid)
                 .attr("onload", "dyniframesize('"+tabid+"')")
                 .attr("src", url)
        ...
        removeTabItem: function (tabid)
        {
            ...
            {
                if($(".l-tab-content-item[tabid=" + tabid + "]", g.tab.content).prev().length == 0){
                	$(".l-tab-content-item[tabid=" + tabid + "]", g.tab.content).next().show();
                	$("li[tabid=" + tabid + "]", g.tab.links.ul).next().addClass("l-selected").siblings().removeClass("l-selected");
                }else{
        ...
        reload: function (tabid)
        {
            ...
            iframe.attr("src", url).unbind('load.tab').bind('load.tab', function ()
            {
                ...
                if($("#content_left").is(":hidden")){
            		$("#content_right").width($("#head").width()-10);
            	}else{
    ...
    l.controls.Window.ligerExtend(l.core.Win, {
        ...
        _render: function ()
        {
            ...
            if (p.url)
            {
                ...
                {
                    var iframe = $("<iframe frameborder='0' src='" + p.url + "' scroling='no'></iframe>");
